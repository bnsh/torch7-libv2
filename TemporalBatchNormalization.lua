--[=[ This is really just a very very simple modification to SpatialBatchNormalization.lua.
      However looking at the newest version of SpatialBatchNormalization.lua, I wonder if
      it's now irrelevant.
--]=]
...
local BN,parent = torch.class('nn.TemporalBatchNormalization', 'nn.Module')
...
function BN:__init(nFeature, eps, momentum, affine)
   ...
   assert(nFeature ~= 0, 'To set affine=false call TemporalBatchNormalization'
...
function BN:updateOutput(input)
   ...
   if self.train == false then
      ...
      self.buffer:repeatTensor(self.running_mean:view(1, 1, nFeature), nBatch, iH, 1)
      self.output:add(-1, self.buffer)
      self.buffer:repeatTensor(self.running_std:view(1, 1, nFeature), nBatch, iH, 1)
      ...
      local in_folded = input:view(nBatch, iH, nFeature)
      self.buffer:mean(in_folded, 1)
      self.buffer2:mean(self.buffer, 2)
      self.running_mean:mul(1 - self.momentum):add(self.momentum, self.buffer2) -- add to running mean
      self.buffer:repeatTensor(self.buffer2:view(1, 1, nFeature),
                               nBatch, iH, 1)
      ...
      local buf_folded = self.buffer:view(nBatch,iH, nFeature)
      self.std:mean(self.buffer2:mean(buf_folded, 1), 2)
      ...
      self.buffer:repeatTensor(self.std:view(1, 1, nFeature),
                               nBatch, iH, 1)
   ...
   if self.affine then
      ...
      self.buffer:repeatTensor(self.weight:view(1, 1, nFeature),
                               nBatch, iH, 1)
      self.output:cmul(self.buffer)
      self.buffer:repeatTensor(self.bias:view(1, 1, nFeature),
                               nBatch, iH, 1)
...
function BN:updateGradInput(input, gradOutput)
   assert(input:dim() == 3, 'only mini-batch supported')
   assert(gradOutput:dim() == 3, 'only mini-batch supported')
   ...
   local gi_folded = self.gradInput:view(nBatch, iH, nFeature)
   self.buffer2:mean(self.buffer:mean(gi_folded, 1), 2)
   self.gradInput:repeatTensor(self.buffer2:view(1, 1, nFeature),
                               nBatch, iH, 1)
   ...
   self.buffer:mean(gradOutput:view(nBatch, iH, nFeature), 1)
   self.buffer2:mean(self.buffer, 2)
   self.buffer:repeatTensor(self.buffer2:view(1, 1, nFeature),
                            nBatch, iH, 1)
...
function BN:accGradParameters(input, gradOutput, scale)
   if self.affine then
      ...
      self.buffer2 = self.buffer2:cmul(gradOutput):view(nBatch, iH, nFeature)
      self.buffer:sum(self.buffer2, 1) -- sum over mini-batch
      self.buffer2:sum(self.buffer, 2) -- sum over pixels
      self.gradWeight:add(scale, self.buffer2)

      self.buffer:sum(gradOutput:view(nBatch, iH, nFeature), 1)
      self.buffer2:sum(self.buffer, 2)
